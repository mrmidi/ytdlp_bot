...
class TwitterDownloader(BaseDownloader):
    """Downloader class for Twitter / X videos using yt-dlp."""
    
    def download(self, url: str, output_dir: Path) -> DownloadResult:
        logger.info(f"Starting Twitter/X download for URL: {url}")
        ...
        ydl_opts = {
            ...
            'quiet': True,
            'no_warnings': True,
            # Use chrome user-agent to bypass basic bot detection
            'http_headers': {
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not downloaded_file:
                raise FileNotFoundError(f"Failed to find downloaded Twitter video file for ID {video_id} in {output_dir}")
            ...
            logger.info(f"Successfully downloaded Twitter video: {title} ({downloaded_file})")
